...
export function ProductCard({ product, categoryName }: ProductCardProps) {
  ...
  return (
    <div className="group relative flex flex-col h-full w-full">
      <Link to={`/produto/${product.id}`} className="relative aspect-[4/5] bg-gray-50 mb-4 overflow-hidden rounded-md block">
        <img
      ...
      <Link to={`/produto/${product.id}`} className="text-center mt-auto block group-hover:text-vinho-700 transition-colors">
        <p className="text-xs text-vinho-700 font-medium mb-1 uppercase tracking-widest">
          {product.categoria_nome || categoryName || 'Produto'}
        </p>
        <h3 className="text-sm text-gray-900 mb-1 inherit">{product.nome}</h3>
